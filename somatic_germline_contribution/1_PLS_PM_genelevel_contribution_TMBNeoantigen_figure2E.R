...
genes_TMB=c("BRCA1","BRCA2","PALB2","MLH1","MSH2","MSH3","MSH6","PMS2","POLE","POLQ","ATM","ATR","CHEK2")
...
subContribution<-fread("./out/PLS-PM_genelevel_contribution_MMR_MSI_genes_20200204.tsv")
...
for(pheno in c("TMB","MSISensor","SNV_Neoantigens","Indel_Neoantigens","Homologous_Recombination_Defects")){
      ...
      #germline FDR 
       contribution$G_FDR=p.adjust(contribution$germline_P,method = "fdr")
       ...
       contribution$G_Sig=ifelse(contribution$G_FDR<0.05,"Significant (FDR<0.05)",contribution$G_Sig)
        #somatic FDR 
       contribution$S_FDR=p.adjust(contribution$somatic_P,method = "fdr")
...
tmp=finalMat[finalMat$Cancer%in%c("UCEC", "COADREAD", "BLCA", "KIRP" ),]
...
th= theme_bw()+ theme(legend.position="none",legend.key.size =unit(.2, "cm"),legend.title = element_text(size=8) ,panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ theme(axis.text.x = element_text(colour="black", size=18,hjust = 0.95), axis.text.y = element_text(colour="black", size=18,hjust = 0.95),axis.ticks = element_blank(),plot.title = element_text(hjust = 0.5,face="bold"),axis.title=element_text(size=18,face="bold"))+ theme(strip.placement = "outside",plot.title = element_text(hjust = 0.5),strip.text.x = element_text(size=14),strip.text.y = element_text(angle = 0,size=10,face="italic"),panel.spacing = unit(1, "lines"),strip.background = element_rect(fill = "#EEEEEE", color = "#FFFFFF"))

#finalMat$Pheno=gsub("Homologous_Recombination_Defects","HRD",finalMat$Pheno)
finalMat=finalMat[-which(finalMat$Pheno%in%c("Homologous_Recombination_Defects","MSISensor")),]
finalMat$Pheno=factor(finalMat$Pheno,levels=c("TMB","SNV_Neoantigens","Indel_Neoantigens"))

finalMat$id=paste0(finalMat$Cancer,"|",finalMat$Pheno)

#add color 
finalMat$color="none"
gsig=finalMat$Cancer[which(finalMat$Type=="Germline" & finalMat$Sig=="Significant (FDR<0.05)")]
ssig=finalMat$Cancer[which(finalMat$Type=="Somatic" & finalMat$Sig=="Significant (FDR<0.05)")]

overlap=intersect(gsig,ssig)
gonly=gsig[!gsig%in%overlap]
sonly=ssig[!ssig%in%overlap]
finalMat$color[finalMat$Cancer%in%gonly]="germline"
finalMat$color[finalMat$Cancer%in%sonly]="somatic"
finalMat$color[finalMat$Cancer%in%overlap]="both"
finalMat$color=factor(finalMat$color,levels=c("both","germline","somatic","none"))
...
finalMat1=finalMat
ids=unique(as.character(finalMat1$id))
for(id in ids){
  index=which(finalMat1$id==id)
  submet=finalMat1[index,]
  if(submet$P[1]>submet$P[2]){
    finalMat1[index[1],"P"]=1
  }else{
    finalMat1[index[2],"P"]=1
  }
}

write.csv(finalMat1,"Figure1E_germlineSomatic_contribution_table.csv")

finalMat1=read.csv("Figure1E_germlineSomatic_contribution_table.csv",h=T,stringsAsFactors = F)

th= theme_bw()+theme(legend.position = "none",axis.text.x = element_text(colour="black", size=16,hjust = 0.95), axis.text.y = element_text(colour="black", size=16,hjust = 0.95),axis.ticks = element_blank(),plot.title = element_text(hjust = 0.5,face="bold"),axis.title=element_text(size=16,face="bold"))+ theme(strip.placement = "outside",strip.text.x = element_text(size=14),strip.text.y = element_text(angle = 0,size=16,face="italic"),panel.spacing = unit(1.5, "lines"),strip.background = element_rect(fill = "#EEEEEE", color = "#FFFFFF"))+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))

finalMat1$Pheno=factor(finalMat1$Pheno,levels=c("TMB","Indel_Neoantigens","SNV_Neoantigens"))

p = ggplot(finalMat1, aes(y=somatic_Contribution, x=germline_Contribution)) 
p = p + geom_point(aes(colour=factor(finalMat1$color),size=-log10(finalMat1$FDR)),alpha=0.6,shape=16)+scale_shape_manual(values=c(1,5))+xlim(-0.1,0.4)# ,shape =Type,alpha = I(0.6)
p = p + geom_text_repel(aes(label=Cancer),size=3.5,segment.alpha =0.5,segment.colour ="grey")
...
#p = p + guides(color=guide_legend(override.aes=list(fill=NA)),linetype=guide_legend(override.aes=list(fill=NA)))+ theme(plot.title = element_text(size=14, face="bold"),axis.title = element_text(size=12), axis.text.x = element_text(colour="black", size=12), axis.text.y = element_text(colour="black", size=12)) 
p = p + labs(size = "-log10(FDR)") +scale_size_continuous(limits=c(0,50),breaks = c(1,10,20,30,40,50))
...
ggsave(fn,w = 9, h = 3, useDingbat=F,limitsize = FALSE)
...
for(pheno in  c("TMB","SNV_Neoantigens","Indel_Neoantigens")){
    ...
    if(mainContribution2$germline_Contribution!=0 & mainContribution2$somatic_Contribution!=0){
      ...
      if(cancer=="UCEC"){
          if(ns==1){
            ...
            elpos_s <-cbind(0.1,seq(from=0.04,to=0.80,by=(0.80-0.04)/(ns-1)))
      ...
      for ( i in 1:ng){
        ...
        text(arrpos_g[1] - 0, arrpos_g[2],cex=1, round(as.numeric(subContribution2[subContribution2$genes==g[i],"contribution"]),digits=2))
      }
      
      for ( i in 1:ns){
        arrpos_s<-straightarrow(from = elpos_s[i, ], to = elpos_l[2, ], lty = 1, lcol = "grey")
        text(arrpos_s[1] + 0, arrpos_s[2],cex=1, round(as.numeric(subContribution2[subContribution2$genes==s[i],"contribution"]),digits=2))
      }
      ...
      text(arrpos_g_f[1] + 0, arrpos_g_f[2],cex=1, round(as.numeric(mainContribution2[,"germline_Contribution"]),digits=2))
      arrpos_s_f=straightarrow(from = elpos_l[2,], to = elpos_f, lty = 1, lcol = "grey")
      text(arrpos_s_f[1] + 0, arrpos_s_f[2],cex=1, round(as.numeric(mainContribution2[,"somatic_Contribution"]),digits=2))
      ...
      if(ns==1){
        ...
        text(arrpos_s[1] + 0, arrpos_s[2],cex=1, round(as.numeric(subContribution2[subContribution2$genes==g[i],"contribution"]),digits=2))
      }else{
...
#TMB
cancer=c("BRCA","OV","COADREAD","PAAD","STAD","UCEC")
pheno="TMB"
...
immuneFeature<-function(label,cancer,pheno,labpos,short_name,gene,col_value,th,w,h){
  ...
  #remove "both" "germline" "somatic" categories with less than 4 patients in each group  
  ...
  for(ca in unique(mutMat$cancer)){
    ...
    for(m in mutype){
      if(length(which(subMat$mut==m))<=3){
        #plab="" 
        next
      }else{
  ...
  #ann_text=rbind(ann_text,cbind(cancer=unique(ann_text$cancer),mut="NA",pheno=pheno,lab=""))
  #ann_text$mut=factor(ann_text$mut,levels=c("both","germline","somatic","NA"))
  ...
  p=p+geom_text(data = ann_text,aes(x=mut,y=labpos,label=ann_text$lab))
  p=p+th
  print(p)
...
immuneFeature("Figure5_",cancer=cancer,pheno=pheno,labpos=500,short_name="TMB",gene=genes,col_value=col_value,th=th,w=8,h=3)


#SNV_Neoantigens
cancer=c("BRCA","COADREAD","UCEC")
pheno="SNV_Neoantigens"
genes=c("BRCA1","BRCA2","PALB2","MLH1","MSH2","MSH3","MSH6","PMS1","PMS2")
col_value=c("both"="#56B4E9","germline"="#56B4E9","somatic"="#56B4E9","NA"="#dbd9d9")
immuneFeature("Figure5_",cancer=cancer,pheno=pheno,labpos=12000,short_name="SNV Neoantigens",gene=genes,col_value=col_value,th=th,w=5,h=3)


#Indel_Neoantigens
cancer=c("BRCA","UCEC","COADREAD","PAAD")
pheno="Indel_Neoantigens"
genes=c("BRCA1","BRCA2","PALB2","MLH1","MSH2","MSH3","MSH6","PMS1","PMS2")
col_value=c("both"="#56B4E9","germline"="#56B4E9","somatic"="#56B4E9","NA"="#dbd9d9")

immuneFeature("Figure5_",cancer=cancer,pheno=pheno,labpos=10000,short_name="Indel Neoantigens",gene=genes,col_value=col_value,th=th,w=5,h=3)
